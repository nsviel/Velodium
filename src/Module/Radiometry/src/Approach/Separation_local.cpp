...
//Method function
bool Separation_local::algo(Collection* collection){
  //---------------------------

  this->algo_ParameterSpace();
  this->algo_checking(cloud);
  this->algo_interpolation(cloud);
...
//Sub-functions
bool Separation_local::algo_checking(Collection* collection){
...
bool Separation_local::algo_interpolation(Collection* collection){
  Cloud* cloud = (Cloud*)*next(collection->list_obj.begin(), 0);
  vector<float>& Is = cloud->I;
  vector<float>& dist = cloud->R;
  vector<float>& cosIt = cloud->cosIt;
...
bool Separation_local::algo_correction(Collection* collection){
  Cloud* cloud = (Cloud*)*next(collection->list_obj.begin(), 0);
  vector<float>& Is = cloud->I;
  Ic.clear();
  //---------------------------
...
MatrixXf Separation_local::kNN_KdTreeFLANN(vector<vec2> PT_PS, vector<vec2> PT_query, bool normalized){
  ...
  //Normalization
  if(normalized){
    vec2 min = fct_min_vec2(PT_PS);
    vec2 max = fct_max_vec2(PT_PS);
...
void Separation_local::plot_RA(){
  ...
  //Write in terminal
  cout<<"fct_normalized parameter space"<<endl;
