...
//frame = vector<Point3D>
//keypoint = list<Point3D>
		...
		for (int iter(0); iter < NUMBER_ITER_CT_ICP; iter++) {
			...
			for (std::list<Point3D>::iterator it_keypoint = keypoints.begin(); it_keypoint != keypoints.end(); ++it_keypoint) {
				...
				if (number_neighbors > MIN_NUMBER_NEIGHBORS) {
					//Find point normal
					{
						//Make a pair with (distance to keypoint, nn point)
						//and sort it according to the distance
						//and take only the 30 nearest nn
						{
					...
					//Fullfill optimization matrices
					{
						if (fabs(dist_to_plane) < max_dist_to_plane) {
							...
							//compute trucs pour l'optimization ok
							{
...
int main(int argc, char **argv){
	for (int seq = 0; seq <= 3; seq++) {
		...
		//some parameter
		{
			...
			trajectory.resize(LAST_INDEX_FRAME_BATVISIONSLAM_ARRAY[seq]);
		...
		for (int index_frame = 0; index_frame < LAST_INDEX_FRAME_BATVISIONSLAM_ARRAY[seq] - FIRST_INDEX_FRAME_BATVISIONSLAM + 1; index_frame++){
			//load chaque frame ok
			{
				std::vector<Point3D> frame;
				std::vector<Point3D> frame_raw;
				...
				for (int i(0); i < numPointsIn; i++) {
					...
					if ((r > MIN_DIST_LIDAR_CENTER) && (r < MAX_DIST_LIDAR_CENTER)) {
						frame_raw.push_back(new_point);
					}
				}
			}

			//Normalize frame timestamp ok
			// alpha_timestamp = normalized timestamp
			{
				trajectory[index_frame].timestamp = frame_first_timestamp;
				delete[] dataIn;

				for (int i(0); i < (int)frame_raw.size(); i++) {
					frame_raw[i].index_frame = index_frame
					machin = (frame_last_timestamp - frame_raw[i].alpha_timestamp) / (frame_last_timestamp - frame_first_timestamp)
					max_truc = max(0.0, 1 - machin)
					frame_raw[i].alpha_timestamp = min(1.0, max_truc);
				}
			}

			//Il y a frame et frame_raw
			// /!\ différence entre frame et frame_raw et point3D point et point_raw ?
			// /!\ le ts de frame_raw est aussi normalizé !!!
			{
				//Copy frame_raw to frame
				for (int i = 0; i < (int)frame_raw.size(); i++) {
					frame.push_back(frame_raw[i]);
				}
			}

			//Save frame avec timestamp normalizé
			{
			...
			//On prend 1 seul point par voxel de taille 0.2
			// frame -> frame subsampled
			{
			...
			//Resave frame avec timestamp normalizé
			{
				if ((index_frame <= 50) || (index_frame % FREQUENCY_SAVE == 0)) {
					stringstream ss_index_frame;
					ss_index_frame << setw(6) << setfill('0') << index_frame + FIRST_INDEX_FRAME_BATVISIONSLAM;
			...
			//update subsampled frame location		ok
			{
				Eigen::Quaterniond q_begin = Eigen::Quaterniond(trajectory[index_frame].begin_R);
				Eigen::Quaterniond q_end = Eigen::Quaterniond(trajectory[index_frame].end_R);
				Eigen::Vector3d t_begin = trajectory[index_frame].begin_t;
			...
			//Grid sampling
			//frame subsampled -> keypoint
			{
				SIZE_VOXEL_KEYPOINTS = 0.3
				...
				sub_sample_frame(frame_sub, SIZE_VOXEL_KEYPOINTS);
			...
			//update location des frames
			//keypoint -> frame & frame_raw
			{
				//Update frame
				Eigen::Quaterniond q_begin = Eigen::Quaterniond(trajectory[index_frame].begin_R);
				Eigen::Quaterniond q_end = Eigen::Quaterniond(trajectory[index_frame].end_R);
				...
				for (int i = 0; i < (int)frame_raw.size(); ++i) {
					double alpha_timestamp = frame_raw[i].alpha_timestamp;
					Eigen::Quaterniond q = q_begin.slerp(alpha_timestamp, q_end);
					q.normalize();
					Eigen::Matrix3d R = q.toRotationMatrix();
					Eigen::Vector3d t = (1.0 - alpha_timestamp) * t_begin + alpha_timestamp * t_end;
					frame_raw[i].pt = R * frame_raw[i].raw_pt + t;
				}
			}
			...
			//Resave frame sans timestamp
			{
				...
				if ((index_frame <= 50) || (index_frame % FREQUENCY_SAVE == 0)) {
					...
					properties.push_back("z"); types.push_back(float32);
					int sizeOfPointOut = 3 * sizeof(float);
					int numPointsOut = (int)frame_raw.size();
					char* dataOut = new char[(unsigned long long int)numPointsOut * (unsigned long long int)sizeOfPointOut];
					int i = 0;
					for (int i(0); i < (int)frame_raw.size(); i++) {
						unsigned long long int offset = (unsigned long long int)i * (unsigned long long int)sizeOfPointOut;
						*((float*)(dataOut + offset)) = (float)frame_raw[i].pt[0]; offset += sizeof(float);
						*((float*)(dataOut + offset)) = (float)frame_raw[i].pt[1]; offset += sizeof(float);
						*((float*)(dataOut + offset)) = (float)frame_raw[i].pt[2]; offset += sizeof(float);
					}
					plyFileOut.writeFile(dataOut, numPointsOut, properties, types);
					delete[] dataOut;
				}
			...
			//Save final cloud
			{
				...
				if (WRITE_POINT_CLOUD) {
					for (int i = 0; i < (int)frame_raw.size(); ++i) {
						point_cloud.push_back(frame_raw[i]);
					}
